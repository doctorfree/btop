...
usage() {
  echo "Usage: sudo ./Install-bin.sh /path/to/btop_<version>-<release>.<arch>.<suffix>"
  echo "Where:"
  ...
  echo "https://github.com/doctorfree/btop/releases"
  exit 1
}
...
[ "${pkgname}" == "btop" ] || {
  ...
  printf "\n\tbtop_<version>-<release>.<arch>.<suffix>\n"
  echo "Distribution archive filename ${ARCHIVE} does not begin with 'btop'"
  usage
}
...
echo "available at https://github.com/doctorfree/btop/releases"
echo ""
while true
do
